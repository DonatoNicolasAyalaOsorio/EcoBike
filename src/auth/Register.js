...
const Register = ({ navigation }) => {
  ...
  const validateFields = () => {
    const { email, contraseña, nombres, apellidos, identificacion, fechaNacimiento, confirmarContraseña } = userData;

    if (!email || !contraseña || !nombres || !apellidos || !identificacion || !fechaNacimiento || !confirmarContraseña) {
  ...
  const handleDateChange = (event, selectedDate) => {
    ...
    if (selectedDate) {
      setUserData((prevData) => ({ ...prevData, fechaNacimiento: selectedDate }));
    }
  };
  ...
  const handleSexSelect = (value) => {
    ...
    setUserData((prevData) => ({ ...prevData, sexo: value }));
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await createUserWithEmailAndPassword(authInstance, userData.email, userData.contraseña);
  
      if (authInstance.currentUser) {
        const userDocRef = doc(db, USERS_COLLECTION, authInstance.currentUser.uid);
        ...
        }, { merge: true }); // Utilizamos merge para actualizar o crear el documento
  ...
  <TouchableOpacity style={styles.input} onPress={showDatepicker}>
    ...
    <Text style={styles.label}>{userData.fechaNacimiento.toDateString()}</Text>
  </TouchableOpacity>
) : (
  ...
  <View style={styles.inputContainer}>
    <TouchableOpacity
      style={[styles.input, styles.textInputStyle, { color: userData.sexo ? 'black' : '#BFBFC1' }]} // Cambia el color del texto según si se seleccionó una opción
      onPress={() => setShowSexModal(true)}
    >
      <Text style={styles.textInputStyle}>Sexo: {userData.sexo}</Text>
    </TouchableOpacity>
  </View>
) : (
  <TextInput
    ...
    autoCorrect={false}
    secureTextEntry={key === 'contraseña' || key === 'confirmarContraseña'}
    keyboardType={key === 'identificacion' ? 'numeric' : 'default'}
    onChangeText={(value) =>
